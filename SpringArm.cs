...
public class SpringArm : MonoBehaviour
{
    ...
    public float mouseSensitivityX = 500f; public float mouseSensiY = 200f;
    ...
    private void Rotate()
    {
        ...
        yaw += Input.GetAxisRaw("Mouse X") * mouseSensitivityX * Time.deltaTime;
        // Decrement pitch by Mouse Y input
        pitch -= Input.GetAxisRaw("Mouse Y") * mouseSensiY * Time.deltaTime;
        // Clamp pitch so that we can't invert the the gameobject by mistake
        pitch = Mathf.Clamp(pitch, -35f, 40f);
